...
export default function ChapterEditor() {
  ...
  // Cargar datos del capítulo al montar el componente
  useEffect(() => {
    loadChapterData();
  }, [storyId, chapterId]);
  ...
  const loadChapterData = async () => {
    try {
      ...
      const storyResponse = await fetch(`/api/v1/stories/${storyId}`);
      const storyData = await storyResponse.json();
      setStoryTitle(storyData.title);
      
      // Encontrar el capítulo específico
      const chapter = storyData.chapters.find(ch => ch.id === parseInt(chapterId));
      if (chapter) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/v1/stories/${storyId}/edit/${chapterId}`, {
        method: 'PUT',
        headers: { 
